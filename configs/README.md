...
| `robot_piper_isaaclab.yaml` | single-arm Piper, Isaac Lab pick-and-lift | `scripts/run_piper_isaaclab_mvtoken.py` |
| `robot_realman_dual_isaaclab.yaml` | dual RM65, Isaac Lab parallel pick-and-place | `scripts/run_realman_dual_isaaclab.py` |
...
| `robot_piper_isaaclab.yaml` | `qwen3_5_2b` (sim adapter; not trained on Piper) | `finetuned_local`, `deepseek` (hosted vision), `heyroute` (Responses API) |
| `robot_realman_dual_isaaclab.yaml` | `qwen3_5_2b` (sim adapter; not trained on RealMan) | `deepseek`, `heyroute`; `--scripted` bypasses all APIs |
